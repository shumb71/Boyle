...
def procesar_peso(session, dias):
    ...
    CONFIRMADO con datos reales (09/09/2026, báscula Renpho vía Health Connect): el valor
    viene en sub["weightGrams"] (gramos, entero, directo — no anidado). Se mantienen los
    candidatos anteriores como fallback por si la API cambia de formato más adelante.
    """
    ...
    for p in puntos:
        ...
        # Confirmado con datos reales (báscula Renpho, 09/09/2026): el valor viene en
        # sub["weightGrams"] (gramos, directo, sin anidar). Se prueba primero.
        peso_kg = None
        if sub.get("weightGrams") is not None:
            g = a_numero(sub.get("weightGrams"))
            peso_kg = g / 1000 if g is not None else None

        # Candidatos anteriores, por si algún día cambia el formato de la API.
        if peso_kg is None:
